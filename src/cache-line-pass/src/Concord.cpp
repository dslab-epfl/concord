...
struct ConcordPass : public ModulePass {
    ...
    bool instrumentLoop(Loop *loop, Function &F, Module &M, LoopInfo &LI, std::string demangledFuncName) {
        ...
        if (!preemptNow) {
            preemptNow = new GlobalVariable(M, Type::getInt32Ty(M.getContext()), false, GlobalValue::ExternalLinkage, 0,
                                            "concord_preempt_now", 0, GlobalValue::GeneralDynamicTLSModel, 0);
        }
...
static void registerConcordPass(const PassManagerBuilder &, legacy::PassManagerBase &PM) { PM.add(new ConcordPass()); }
